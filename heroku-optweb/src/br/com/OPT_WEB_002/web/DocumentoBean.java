...
public class DocumentoBean implements Serializable {
	...
	public List<Documento> listarPorIdTipoDocCodEmpCodFiCodUni(Usuario usuario) {
		...
		if (usuario != null) {
			...
			if (usuario.isTipo_usuario() && id_tipo_doc != null) {
				...
				return documentoRN.listarPorIdTipoDocCodEmpCodFiCodUni(id_tipo_doc,
						usuario.getCod_empresa().getCod_empresa(), usuario.getCod_filial().getCod_filial(),
						usuario.getCod_unidade().getCod_unidade());
				...
				for(Layout_Empresa layout_Empresa : layout_EmpresaRN.listarCampoReferencia(id_tipo_doc,usuario.getCod_empresa().getCod_empresa(),usuario.getCod_filial().getCod_filial(),usuario.getCod_unidade().getCod_unidade())){
				
					for(Documento documento : documentoRN.listarPorIdTipoDocCodEmpCodFiCodUni(layout_Empresa.getId_tipo_doc().getId_tipo_doc(),layout_Empresa.getCod_empresa().getCod_empresa(),layout_Empresa.getCod_filial().getCod_filial(),layout_Empresa.getCod_unidade().getCod_unidade())){
						
						try {
							
							if(documento.getClass().getDeclaredField(layout_Empresa.getCod_campo()) != null){
													
								Field campo =	documento.getClass().getDeclaredField(layout_Empresa.getCod_campo());
								campo.setAccessible(true);
									
										if(usuario.getCliente().equals(campo.get(documento))){
	...
	public Documento iniciar(Usuario usuario) throws IllegalArgumentException, IllegalAccessException {
		...
		if (id == null) {
			...
			try{
				if(layout_EmpresaRN.carregarPorCodCampo("int_001",usuario.getCod_empresa().getCod_empresa(),usuario.getCod_filial().getCod_filial(),usuario.getCod_unidade().getCod_unidade()).isFlag_campo() && layout_EmpresaRN.carregarPorCodCampo("int_001",usuario.getCod_empresa().getCod_empresa(),usuario.getCod_filial().getCod_filial(),usuario.getCod_unidade().getCod_unidade()) != null){
					...
					desabilitaCampoIncremental("int_001",documento);
				}
	...
	public String salvar() throws DAOException {
		...
		if (id == null) {
			...
			return "/restrito/documento/documento.xhtml?id=" + this.documento.getId_tipo_doc().getId_tipo_doc()
					+ "faces-redirect=true";
		}
	...
	public StreamedContent getStreamedContentDoc() {
		...
		if (documento.getArquivo() != null) {
			...
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_WARN, "Warning!", "O arquivo nao existe"));
			return null;
		}
	}
